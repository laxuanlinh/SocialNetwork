    ...
    <div class="media-body">
        ...
        <ul class="list-inline">
            ...
            @if(!Auth::user()->hasLikedStatus($status))
                <li><a href="{{route('status.like', ['statusId'=>$status->sid])}}">Like</a></li>
            @else
                <li><a href="{{route('status.dislike', ['statusId'=>$status->sid])}}">Dislike</a></li>
            @endif
                ...
                <div class="media-body">
                    ...
                    <ul class="list-inline">
                        ...
                        @if(!Auth::user()->hasLikedStatus($reply))
                            <li><a href="{{route('status.like', ['statusId'=>$reply->sid])}}">Like</a></li>
                        @else
                            <li><a href="{{route('status.dislike', ['statusId'=>$reply->sid])}}">Dislike</a></li>
                        @endif
        ...
        <form role="form" action="{{route('status.reply', ['statusId'=>$status->sid])}}" method="post">
                ...
                <div class="form-group has-error">
                    @else
                        <div class="form-group">
                            @endif
                            <textarea name="reply-{{$status->sid}}" class="form-control" rows="2" placeholder="Reply to this status"></textarea>
                            @if($errors->has("reply-{$status->sid}"))
